...
if(p==1)
    ...
    MM=  single(zeros(nk*(numG+2*(Lx1)),nk*(numG+2*(Lx1))));

%         for kx=1:size(MM,1)
%         for ky=1:size(MM,2)
%             MM(kx,ky)=single(1e-10+1i*1e-10);
%         end
%     end
        ...
        for Gy=1:nGy
            ...
            for Gx1=-nGx:nGx
                ...
                for Gy1=1:nGy
                    ...
                    %  AkdKap=dKapTens*Akg;
                    AkdKap=1*(KapTenss*Akg-KapTensd*(Akg));
                    ...
                    Akg2=pp*eye(3,3)-Akg1;
                    %dKapTens2=dKapTens*Akg2;
                ...
                for j=1:nk
                    for k=1:nk
                        ...
                        MM((countG-1)*nk+j,(numG+Gxp-1)*nk+k)=tempT(j,k);
                        MM((countG-1)*nk+j,(numG+dd+Gxp-1)*nk+k)=tempR(j,k);
                ...
                if(Gx==Gx1)
                    for k=1:nk
                        if(k~=2)
                            ...
                            MM((Gxp+numG-1)*nk+k,((Gxp-1)*nGy+Gy-1)*nk+k)=pi*Gy*sc;
                        end
                    end
                    if(mod(Gy,2)==0)
                        for k=1:nk
                            if(k~=2)
                                MM((Gxpp+numG-1)*nk+k,((Gxp-1)*nGy+Gy-1)*nk+k)=pi*Gy*sc;
                            end
                        end
                    else
                        for k=1:nk
...
for Gx=-nGx:nGx
    ...
    for k=1:nk
        ...
        if(twoMat)
            ...
            if(k~=2)
                MM((numG+Gxp-1)*nk+k,(numG+Gxp-1)*nk+k)=1*sc;
                
                MM((numG+Gxp-1)*nk+k,(numG+Gxpp-1)*nk+k)=-(1i*L*krny+1)*sc;
                
                
                MM((numG+Gxpp-1)*nk+k,(numG+Gxp-1)*nk+k)=-(1i*L*ktny-1)*sc;
                
                MM((numG+Gxpp-1)*nk+k,(numG+Gxpp-1)*nk+k)=-1*sc;
            else
        ...
        if(Gx==0)
            ...
            if(k~=2)
            bN((numG+Gxp-1)*nk+k)= (1i*L*k1y+1)*E0(k)*sc;
            bN((numG+Gxpp-1)*nk+k)=E0(k)*sc;
            end
        end
    end
end
...
nL=max(int32(L*k1/2/pi)*17*8,35);
